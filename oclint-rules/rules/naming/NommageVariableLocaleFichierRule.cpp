...
class NommageVariableLocaleFichierRule : public AbstractASTVisitorRule<NommageVariableLocaleFichierRule>
{
private:
...
public:
    ...
    {
        return "nommage variable locale fichier";
    }
    ...
    bool VisitVarDecl(VarDecl *node)
    {
		...
		bool isStatic = (node->getStorageClass() == SC_Static);

    	// Si on a une variable globale (locale au fichier) et statique
    	if(!(node->isLocalVarDecl()) && isStatic )
		{
			// préfixe "s_" ou "g_"
			const char *pattern = "[gs]_\\w{1,}";
...
static RuleSet rules(new NommageVariableLocaleFichierRule());
